...
# 2. Supabase insert (if key available)
SUPABASE_URL = os.environ.get('SUPABASE_URL', 'https://bwspcsiazbwrrxpgoldx.supabase.co')
SUPABASE_KEY = os.environ.get('SUPABASE_ANON_KEY', '')
if SUPABASE_KEY and SUPABASE_KEY != 'YOUR_SUPABASE_ANON_KEY_OR_SERVICE_ROLE_KEY':
    ...
    try:
        with urllib.request.urlopen(req, timeout=15) as resp:
            print(f'[SUPA] HTTP {resp.status} — inserted to cron_results')
    except Exception as e:
        print(f'[SUPA] Failed: {e}')
else:
    print('[SUPA] Skipped — no Supabase key in environment')
