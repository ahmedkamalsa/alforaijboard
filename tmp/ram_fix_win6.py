...
# RAM — try free -m first (this is the one that worked earlier in the session)
ram_status = "FAIL"
ram_detail = r["checks"]["ram"]["detail"]

try:
    r2 = subprocess.run(["free", "-m"], capture_output=True, text=True, timeout=5, shell=True)
    print("free -m rc:", r2.returncode)
    print("free -m stdout:", repr(r2.stdout[:500]))
    print("free -m stderr:", repr(r2.stderr[:500]))
    if r2.returncode == 0 and "Mem:" in r2.stdout:
        ...
        print("free -m: no Mem line or non-zero rc")
