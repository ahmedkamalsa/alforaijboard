...
add_check("disk", disk_status, f"{round(free_gb,1)} GB free ({free_pct}%) / {round(total_gb,1)} GB total — threshold: 10% free")
...
# --- Overall ---
docker_ok = report["checks"]["docker"]["status"] in ("RUNNING",)
...
# --- Supabase push (TWO COLUMNS ONLY per actual schema: name, status, message, duration_ms, records_affected, created_at) ---
...
print(f"\n[DB] Pushing to Supabase cron_results...")
...
try:
    ...
    with urllib.request.urlopen(req, timeout=15) as resp:
        ...
        print(f"\n[DB] SUCCESS: HTTP {resp.status}")
        print(f"[DB] Body: {body}")
except urllib.error.HTTPError as e:
    print(f"\n[DB] HTTP ERROR: {e.code} {e.reason}")
