#!/usr/bin/env python3
"""Hourly system health check - Windows. Saves local JSON + pushes to Supabase."""
...
report["checks"]["hermes_agents"] = {"status": agent_status, "severity": "info", "details": f"{agent_count} Hermes-related process(es) found. PIDs: {pids if pids else 'none'}"}
...
if SERVICE_KEY:
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=15) as resp:
            ...
            db_status = f"INSERTED HTTP {resp.status}"
            print(f"\n[DB] Supabase insert: {db_status} — {body[:200]}")
    except Exception as exc:
        db_status = f"FAILED (service key): {exc}"
        print(f"\n[DB] Supabase insert failed: {exc}")
else:
    db_status = "SKIPPED (no service key)"
print(f"\n[DB] {db_status}")
