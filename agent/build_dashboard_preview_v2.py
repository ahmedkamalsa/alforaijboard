  ...
  <script id="professional-preview-v2-script">
    ...
    function governorateMetricCount(governorate, metric) {
      return records.filter(row =>
        ...
        rowMatchesSharedFilters(row, false, true) &&
    ...
    function renderProfessionalGovernorTable() {
      ...
      Object.values(governorateNameMap).forEach(governorate => {
        ...
        nameCell.textContent = governorate;
        tr.appendChild(nameCell);
        comparisonMetrics.forEach(metric => {
          const count = governorateMetricCount(governorate, metric);
    ...
    function printRowsTable(title, rows) {
      ...
      return `
        <section>
          ...
          <table>
            <thead><tr><th>الكود</th><th>المحافظة</th><th>المنطقة</th><th>نوع العقار</th><th>السعر</th><th>التاريخ</th><th>النمط</th></tr></thead>
            <tbody>${body || '<tr><td colspan="7">لا توجد سجلات مطابقة.</td></tr>'}</tbody>
          </table>
        </section>`;
    }
...
def build_preview() -> Path:
    ...
    html = html.replace("</body>", preview_js + "\n</body>", 1)
