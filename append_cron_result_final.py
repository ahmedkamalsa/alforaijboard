#!/usr/bin/env python3
"""Append this hour's health report to cron_results_local.json (Kuwait-time run) — with defensive parsing."""
...
# Defensive load: ensure existing is a list of dicts
try:
    ...
    if isinstance(raw, list) and all(isinstance(e, dict) for e in raw):
        ...
        # Re-wrap if someone wrote a malformed structure
        existing = [raw] if isinstance(raw, dict) else []
except Exception:
    existing = []

if not any(isinstance(e, dict) and e.get("cron_id") == new_entry["cron_id"] for e in existing):
